...
protected:
    ...
    {
        rd_kafka_resp_err_t           code = RD_KAFKA_RESP_ERR_NO_ERROR;
        std::string                  reason;
        std::chrono::milliseconds    age{0};
    };

    Optional<LastErrorInfo> lastErrorInfo() const;
...
private:
    ...
    void recordError(rd_kafka_resp_err_t code, const char* reason);

    mutable std::mutex  _lastErrorMutex;
    rd_kafka_resp_err_t _lastErrorCode = RD_KAFKA_RESP_ERR_NO_ERROR;
    std::string         _lastErrorReason;
    std::chrono::steady_clock::time_point _lastErrorAt{};
                         ...
                         const ConfigCallbacksRegister& extraConfigRegister)
{
    ...
    // Set client handler
    auto* errStr = errInfo.clear().str();
    const auto errCapacity = errInfo.capacity();
    rd_kafka_t* rkRaw = rd_kafka_new((clientType == ClientType::KafkaConsumer ? RD_KAFKA_CONSUMER : RD_KAFKA_PRODUCER),
                                     rk_conf.release(),  // rk_conf's ownship would be transferred to rk, after the "rd_kafka_new()" call
                                     errStr,
                                     errCapacity);
    if (!rkRaw)
    {
        const std::string errMsg = (errStr && errStr[0] != '\0') ? errStr : "unknown error";
        KAFKA_THROW_ERROR(Error(rd_kafka_last_error(), std::string("Failed to create Kafka client: ").append(errMsg)));
    }
    _rk.reset(rkRaw);
...
inline void
KafkaClient::recordError(rd_kafka_resp_err_t code, const char* reason)
{
    ...
    std::lock_guard<std::mutex> guard(_lastErrorMutex);
    _lastErrorCode = code;
    _lastErrorReason = reason;
    _lastErrorAt = std::chrono::steady_clock::now();
    _hasLastError = true;
}

inline Optional<KafkaClient::LastErrorInfo>
KafkaClient::lastErrorInfo() const
{
    std::lock_guard<std::mutex> guard(_lastErrorMutex);
    if (!_hasLastError)
    {
        return Optional<LastErrorInfo>{};
    }

    const auto age = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - _lastErrorAt);
    return LastErrorInfo{_lastErrorCode, _lastErrorReason, age};
}
...
KafkaClient::errorCallback(rd_kafka_t* rk, int err, const char* reason, void* /*opaque*/)
{
    ...
    if (respErr != RD_KAFKA_RESP_ERR__FATAL)
    {
        ...
        kafkaClient(rk).recordError(respErr, reason);
    }
    else
    {
        LogBuffer<LOG_BUFFER_SIZE> errInfo;
        ...
        kafkaClient(rk).recordError(respErr, errInfo.c_str());
    }
