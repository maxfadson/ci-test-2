...
public:
    ...
    std::string     message()     const
    {
        ...
        std::string base = _rkError ? rd_kafka_error_string(_rkError.get()) : rd_kafka_err2str(_respErr);
        if (const char* detail = extraDetail(static_cast<rd_kafka_resp_err_t>(value()))) {
            if (base.find(detail) == std::string::npos) {
                base.append(" (").append(detail).append(")");
            }
        }
        return base;
    }
...
private:
    static const char* extraDetail(rd_kafka_resp_err_t err)
    {
        switch (err) {
        case RD_KAFKA_RESP_ERR__MSG_TIMED_OUT:
            return "No ack received within the time limit";
        default:
            return nullptr;
        }
    }
