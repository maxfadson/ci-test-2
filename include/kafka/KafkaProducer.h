...
private:
    ...
    public:
        ...
        {
            Error error{rkmsg->err};
            if (rk && rkmsg->err == RD_KAFKA_RESP_ERR__MSG_TIMED_OUT)
            {
                auto* producer = static_cast<KafkaProducer*>(rd_kafka_opaque(rk));
                if (producer)
                {
                    if (auto info = producer->lastErrorInfo())
                    {
                        if (!info->reason.empty())
                        {
                            std::string msg = "No ack received within the time limit. Last error: " + info->reason
                                              + " (last seen " + formatErrorAge(info->age) + " ago)";
                            error = Error{rkmsg->err, std::move(msg)};
                        }
                    }
                }
            }
    ...
    private:
        static std::string formatErrorAge(std::chrono::milliseconds age)
        {
